...
func (handler *MongoConnectionHandler) GetWorkoutAggregation(ctx *gin.Context) {
	...
	cursor, err := handler.collection.Find(ctx, filter, findOptions)
	if err != nil {
	...
	for _, entry := range results {
		for _, exercise := range entry.Workout.Exercises {
			for _, set := range exercise.Sets {
				point := map[string]interface{}{
					"x": entry.PublishedAt.Format("2006-01-02"),
					"y": set.Weight,
				}
				data[exercise.Name] = append(data[exercise.Name], point)
			}
		}
	}
	...
	ctx.JSON(http.StatusOK, gin.H{"datasets": datasets})
}
